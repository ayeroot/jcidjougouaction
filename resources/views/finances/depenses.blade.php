...
@php
    ...
    $catBadge = ['projet'=>'bg-cyan-100 text-cyan-700','prestation'=>'bg-purple-100 text-purple-700','secretariat'=>'bg-amber-100 text-amber-700','fonctionnement'=>'bg-slate-100 text-slate-600','autre'=>'bg-slate-100 text-slate-600'];
@endphp
    ...
    <div class="bg-white border rounded-xl p-6 h-fit">
        ...
        <form method="POST" action="{{ route('finances.depenses.store') }}" class="space-y-3">
            ...
            <div>
                ...
                <select name="categorie" required class="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-jci-600 outline-none">
                    @foreach ($cats as $v=>$l)<option value="{{ $v }}">{{ $l }}</option>@endforeach
                </select>
            </div>
            <div>
                <label class="block text-sm font-medium mb-1">Projet (facultatif)</label>
